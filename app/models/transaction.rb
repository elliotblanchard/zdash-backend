class Transaction < ApplicationRecord
  ...
  def self.classify(zhash)
    ...
    if transaction
      if transaction.vin.length.positive?
        ...
        if parsed[0].length > 18
          ...
          if transaction.vout.length.positive?
          ...
          if transaction.vout.length.positive?
            ...
            if transaction.vjoinsplit.length.positive?
              'sprout_shielding'
            else
              'sapling_shielding'
            end
        ...
        if transaction.vout.length.positive?
          if transaction.vjoinsplit.positive?
            'sprout_deshielding'
          else
            'sapling_deshielding'
          end
    ...
    # Also need to add migration for classification column in the db
